class FisheyeQuad {
  ...
  float Z_INDEX_FACTOR = 0.01f;
  int BILLBOARD_MODE = 0;
  int CYLINDER_MODE = 1;
  float RESOLUTION_FACTOR = 1/30f; //one segment per n pixels.
  ...
  void setRes(float theWidth, float theHeight) {
    hRes = floor(theWidth*RESOLUTION_FACTOR);
    vRes = floor(theHeight*RESOLUTION_FACTOR);
  }
}
