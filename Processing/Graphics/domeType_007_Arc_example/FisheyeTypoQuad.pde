class FisheyeTypoQuad extends FisheyeQuad {
  ...
  void initTex(int texW, int texH) {
    ...
    for (int i = 0; i < hRes+1; i++) {
      for (int j = 0; j < vRes+1; j++) {
        texels[i][j][0] = (i/(hRes+0f)) * tex.width;
        texels[i][j][1] = (1f - j/(vRes+0f))*tex.height;
      }
    }
  }
  ...
  @Override
    ...
    if (debug) {
      ...
      //p5.noStroke();
      p5.pushMatrix();
      p5.translate(p5.width*0.5f,p5.height*0.5f,-p5.height);
      p5.scale(0.5f,0.5f,0.5f);
      p5.translate(-p5.width*0.5f,-p5.height*0.5f,0);
      p5.rotateX(QUARTER_PI);
      p5.rotateY(QUARTER_PI);
      for (int j = 0; j < vRes; j++) {
        p5.beginShape(QUAD_STRIP);
        p5.texture(tex);
        for (int i = 0; i < hRes+1; i++) {
          p5.vertex(vertices[i][j][0],vertices[i][j][1],vertices[i][j][2],texels[i][j][0],texels[i][j][1]);
          p5.vertex(vertices[i][j+1][0],vertices[i][j+1][1],vertices[i][j+1][2],texels[i][j+1][0],texels[i][j+1][1]);
        }
        p5.endShape();
      }
      p5.popMatrix();
