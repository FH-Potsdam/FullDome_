 ...
 */
public class CubeMapUtilsCubes {

    FullDomeGLCubes parent;
    PApplet p;
    GLSLShader shader;
    PGraphicsOpenGL pgl;
    GL gl;
    ...
    public CubeMapUtilsCubes(FullDomeGLCubes _parent){
    ...
    void initCubeMap() {
        ...
        shader = new GLSLShader(p, "cubemapvert.glsl", "cubemapfrag.glsl");
   }
    ...
    void drawDomeMaster() {
        // draw the ellipse
        p.ortho(-p.width * .025f, p.width * .025f, -p.height * .025f, p.height * .025f,
                -1500, 1500);

        p.camera(0, 0, 150, // <- NOTE: this is dynamic and changes the
        ...
        glut.glutSolidSphere(p.height*0.03, 50, 50);
